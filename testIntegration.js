 ...
 *
 * This script tests the integration of the FLL Competition Scheduler with the website.
 */

// Import the CommonJS adapter using dynamic import
...
console.log(`Number of teams: ${Object.keys(teamsSchedule).length}`);
...
console.log("\nTable 1 Schedule:");
if (tableSchedule[0]) {
  for (const event of tableSchedule[0]) {
    const startHour = Math.floor(event.startTime / 60);
    const startMinute = event.startTime % 60;
    const endHour = Math.floor((event.startTime + event.duration) / 60);
    const endMinute = (event.startTime + event.duration) % 60;
    ...
    console.log(`  ${startTimeStr} - ${endTimeStr}: Team ${event.teamID}`);
  }
} else {
  console.log("  No events found for Table 1");
}

console.log("\nJudging Room 1 Schedule:");
if (judgingSchedule[0]) {
  for (const event of judgingSchedule[0]) {
    const startHour = Math.floor(event.startTime / 60);
    const startMinute = event.startTime % 60;
    const endHour = Math.floor((event.startTime + event.duration) / 60);
    const endMinute = (event.startTime + event.duration) % 60;
    ...
    console.log(
      `  ${startTimeStr} - ${endTimeStr}: Team ${event.teamID} (${eventTypeStr})`
    );
  }
} else {
  console.log("  No events found for Judging Room 1");
}

console.log("\nIntegration test completed successfully!");
console.log("To test the integration with the website, run:");
